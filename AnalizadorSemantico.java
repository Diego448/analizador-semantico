...
import analizadorSemantico.LeerDocumento;
...
public class AnalizadorSemantico {
	...
	static JTextArea resultadoAnalisisSemantico = new JTextArea();
	...
	public static void inicializarVentana() {
		...
		resultadoAnalisisSintactico.setPreferredSize(new Dimension(280, 150));
		resultadoAnalisisSemantico.setPreferredSize(new Dimension(280, 45));
		ventanaPrincipal.setResizable(false);
		ventanaPrincipal.getContentPane().add(etiquetaRutaDeArchivo);
		ventanaPrincipal.getContentPane().add(rutaDeArchivo);
		rutaDeArchivo.setText("Prueba.txt");
		...
		ventanaPrincipal.getContentPane().add(resultadoAnalisisSemantico);
			...
			switch(instrucciones[instruccionActual][simboloActual]) {
				...
				case IDENTIFICADOR:
					...
					if(instrucciones[instruccionActual][simboloActual] == TablaDeSimbolos.OPERADOR_DE_ASIGNACION) {
						...
						} else if(instrucciones[instruccionActual][simboloActual] == TablaDeSimbolos.OPERADOR_ARITMETICO) {
	...
	public static void analisisSemantico(String codigo) {
		...
		if(codigo.trim().matches("[+-/*]\\s\\d+\\s\\d+") || codigo.trim().matches("[+-/*]\\s[+-/*]\\s\\d+\\s\\d+\\s[+-/*]\\s\\d+\\s\\d+")) {
